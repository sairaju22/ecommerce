...
def additems():
    if session.get('admin'):
        if request.method=="POST":
            ...
            idotp=itemidotp()
            filename=idotp+'.jpg'
            cursor.execute('insert into additems(itemid,name,des,qty,category,price)\
...
def pay(itemid,name,price):
    try:
        qyt=int(request.form['qty'])
        total_price= int(price)* qyt
        ...
        return render_template('pay.html',order=order,itemid=itemid,name=name,price=total_price,qty=qyt)
...
def search():
        if request.method=='POST':
            ...
            cursor=mydb.cursor()
            cursor.execute('select* from add items wherename=%s',[name])
            data=cursor.fetchall()
